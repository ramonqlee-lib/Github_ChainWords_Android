...
@interface WordChainController ()<UITableViewDataSource,UITableViewDelegate,KeyBordVIewDelegate,ChartCellDelegate,AVAudioPlayerDelegate>
...
@implementation WordChainController
...
-(void)KeyBordView:(KeyBordVIew *)keyBoardView textFiledReturn:(UITextField *)textFiled
{
    ...
    //TODO 自动生成输入单词
    //FIXME:: 自动根据当前的单词，添加下一个接龙的单词
    [self addNextWithMessage:chartMessage];
...
#pragma add new word for chaining
-(void)addNextWithMessage:(ChartMessage*)currentMsg
{
    ChartCellFrame *cellFrame=[[ChartCellFrame alloc]init];
    ChartMessage *chartMessage=[[ChartMessage alloc]init];
    
    // 在此输入文本
    WordsRepository* rep = [WordsRepository sharedInstance];
    [rep applyClassifier:[HeadClassifier new]];
    
    NSString* nextWord = [rep next:currentMsg.content duplicate:NO];
    // add to cell
    if (!nextWord || 0 == nextWord.length) {
        // match failed,we should change a word
        [self.view makeToast:@"我已经黔驴技穷了，咱们换个单词吧！^_^"];
        return;
    }
    
    chartMessage.content = nextWord;
    chartMessage.messageType=(kMessageFrom==currentMsg.messageType)?kMessageTo:kMessageFrom;
    chartMessage.icon= (kMessageFrom == chartMessage.messageType)? kIconFrom : kIconTo;
    cellFrame.chartMessage=chartMessage;
