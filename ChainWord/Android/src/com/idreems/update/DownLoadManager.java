...
public class DownLoadManager {
    ...
    public static final String FILE_DIR = new StringBuilder(Environment.getExternalStorageDirectory().getAbsolutePath()).append(File.separator).append(DOWNLOAD_FOLDER_NAME).append(File.separator).toString();
	...
	public void setOnDownLoadResultListener(OnDownLoadResultListener downLoadResultListener) {
	 ...
	 * @param context 
	 * @param url 下载apk的url
	 * @param md5Vaiue MD5校验值
	 * @param notificationTitle 通知栏标题
	 * @param notificationDescription 通知栏描述
	 */
    public DownLoadManager(Context context, String url, String md5Vaiue , String notificationTitle,
		...
		apkDownloadId = url.substring(url.lastIndexOf("/")+1 , url.lastIndexOf("."));
...
//		putDownLoadStatus(true);
		...
		if(status == DownloadManager.STATUS_PENDING || status == DownloadManager.STATUS_RUNNING || status == DownloadManager.STATUS_PAUSED){ //说明正在下载
			return;
		}
...
//    	SharedPreferences settings = context.getSharedPreferences(UPDATESTATUS, Context.MODE_PRIVATE);
//        SharedPreferences.Editor editor = settings.edit();
